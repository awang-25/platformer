...
public class Level {
	...
	//Precondition: player touches flower within map bounds, numSquaresToFill > 0
	//Postcondition: gas comes out of flower in a certain order
	//[col, row-1], [col+1, row-1], [col-1, row-1], [col+1, row], [col-1, row], [col, row+1], [col+1, row+1], [col-1, row+1]
	private void addGas(int col, int row, Map map, int numSquaresToFill, ArrayList<Gas> placedThisRound) {
		...
		while(placedThisRound.size() > 0 && numSquaresToFill > 0){
			...
			if(row-1 >= 0 && !(map.getTiles()[col][row-1].isSolid()) && !(map.getTiles()[col][row-1] instanceof Gas) && numSquaresToFill > 0){
				g = new Gas(col, row-1, tileSize, tileset.getImage("GasOne"), this, 0);
				map.addTile(col, row-1, g);
				numSquaresToFill--;
				placedThisRound.add(g);
			}
			if(col+1 < width && row-1 >= 0 && !(map.getTiles()[col+1][row-1].isSolid()) && !(map.getTiles()[col+1][row-1] instanceof  Gas) && numSquaresToFill > 0){
				g = new Gas(col+1, row-1, tileSize, tileset.getImage("GasOne"), this, 0);
				map.addTile(col+1, row-1, g);
				numSquaresToFill--;
				placedThisRound.add(g);
			}
			if(col-1 >= 0 && row-1 >= 0 && !(map.getTiles()[col-1][row-1].isSolid()) && !(map.getTiles()[col-1][row-1] instanceof  Gas) && numSquaresToFill > 0){
				g = new Gas(col-1, row-1, tileSize, tileset.getImage("GasOne"), this, 0);
				map.addTile(col-1, row-1, g);
				numSquaresToFill--;
				placedThisRound.add(g);
			}
			if(col+1 < width && !(map.getTiles()[col+1][row].isSolid()) && !(map.getTiles()[col+1][row] instanceof  Gas) && numSquaresToFill > 0){
				g = new Gas(col+1, row, tileSize, tileset.getImage("GasOne"), this, 0);
				map.addTile(col+1, row, g);
				numSquaresToFill--;
				placedThisRound.add(g);
			}
			if(col-1 >= 0 && !(map.getTiles()[col-1][row].isSolid()) && !(map.getTiles()[col-1][row] instanceof  Gas) && numSquaresToFill > 0){
				g = new Gas(col-1, row, tileSize, tileset.getImage("GasOne"), this, 0);
				map.addTile(col-1, row, g);
				numSquaresToFill--;
				placedThisRound.add(g);
			}
			if(row+1 < height && !(map.getTiles()[col][row+1].isSolid()) && !(map.getTiles()[col][row+1] instanceof  Gas) && numSquaresToFill > 0){
				g = new Gas(col, row+1, tileSize, tileset.getImage("GasOne"), this, 0);
				map.addTile(col, row+1, g);
				numSquaresToFill--;
				placedThisRound.add(g);
			}
			if(col+1 < width && row+1 < height && !(map.getTiles()[col+1][row+1].isSolid()) && !(map.getTiles()[col+1][row+1] instanceof  Gas) && numSquaresToFill > 0){
				g = new Gas(col+1, row+1, tileSize, tileset.getImage("GasOne"), this, 0);
				map.addTile(col+1, row+1, g);
				numSquaresToFill--;
				placedThisRound.add(g);
			}
			if(col-1 >= 0 && row+1 < height && !(map.getTiles()[col-1][row+1].isSolid()) && !(map.getTiles()[col-1][row+1] instanceof  Gas) && numSquaresToFill > 0){
				g = new Gas(col-1, row+1, tileSize, tileset.getImage("GasOne"), this, 0);
				map.addTile(col-1, row+1, g);
				numSquaresToFill--;
				placedThisRound.add(g);
